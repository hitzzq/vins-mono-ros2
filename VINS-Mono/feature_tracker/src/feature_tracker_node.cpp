...
//rclcpp::Publisher<>::SharedPtr pub_img,pub_match;
//rclcpp::Publisher<>::SharedPtr pub_restart;
...
void img_callback(const sensor_msgs::msg::Image::ConstPtr img_msg)
{
    if(first_image_flag)
    {
    ...
    {
        std::cout<<"image discontinue! reset the feature tracker!"<<std::endl;
   ...
   if (PUB_THIS_FRAME)
   {
        ...
        printf("publish %f, at %f", feature_points->header.stamp.sec+feature_points->header.stamp.nanosec * (1e-9), rclcpp::Clock().now().nanoseconds()*(1e-9));
    ...
    printf("whole feature tracker processing costs: %f", t_r.toc());
}
